...
export function UsageForCommand(
  ...
  const { positionalParser, namedParsers, escapedParser, description } = current;
  ...
  if (namedParsers) {
    ...
    if (namedParserEntries.length > 0) {
      ...
      lines.push('Options:');
      lines.push('');
      const rows: TwoColumnTableRow[] = namedParserEntries.map(([name, parser]) => {
        let cell0 = `--${name}`;
  ...
  if (escapedParser && !escapedParser.hidden) {
    ...
    firstLine += ` ${required ? fullPlaceholder : wrapInSquareBrackets(fullPlaceholder)}`;
